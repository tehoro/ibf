...
DEFAULT_CONTEXT_LLM = "gemini-3.5-flash-lite"
# Before 0.8, this model's cache filenames omitted the model suffix. Keep that
# convention only for explicitly configured legacy-model caches so the new
# default cannot silently reuse context generated by a different model.
