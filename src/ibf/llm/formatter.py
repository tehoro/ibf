...
def _member_summary(
    ...
    *,
    ...
    if include_temperatures and not (math.isfinite(high_temp) and math.isfinite(low_temp)):
...
def _normalize_daily_total(value: float, unit: str, *, kind: str) -> float:
    ...
    For mm rainfall, omit totals below 1 mm and round reportable totals to whole
    millimetres. Sub-reportable rain can still be described from the hourly data.
    """
    if not isinstance(value, (int, float)):
        return 0.0
    v = float(value)
...
def _format_total_amount_line(value: float, unit: str, *, label: str) -> str:
    ...
    For mm rainfall, omit totals below 1 mm and round larger totals to the
    ...
    if label == "rainfall" and unit == "mm":
        ...
        if rounded <= 0:
            return ""
        return f" Total rainfall: {int(rounded)} {unit_label}."

    # Default formatting: keep existing behavior but avoid "0.0".
    precision = 0 if unit == "mm" else 1
    rounded = round(v, precision) if precision == 0 else _round_half_up(v * 10.0) / 10.0
    if rounded == 0:
        return ""
    if precision == 0:
