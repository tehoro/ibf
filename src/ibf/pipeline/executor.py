...
class ForecastGenerationFailure(RuntimeError):
    """Signal that a forecast page could not be generated by any configured LLM."""


class PipelineRunError(RuntimeError):
    """Report forecast-generation failures after the rest of the pipeline has run."""

    def __init__(self, failures: Iterable[str]):
        self.failures = tuple(failures)
        super().__init__(
            f"{len(self.failures)} forecast generation failure(s): "
            + "; ".join(self.failures)
        )
...
def execute_pipeline(config: ForecastConfig) -> None:
    ...
    for i, location in enumerate(config.locations):
        ...
        except ForecastGenerationFailure as exc:
    ...
    _log_cost_summary()
    if failures:
        raise PipelineRunError(failures)
...
def _process_location(location: LocationConfig, config: ForecastConfig, display_name: Optional[str] = None) -> Optional[LocationForecastPayload]:
    ...
    if not forecast_text:
        ...
        _preserve_or_render_unavailable_forecast(
        ...
        raise ForecastGenerationFailure(
            f"Location forecast unavailable for {unique_name}; all configured LLM attempts failed."
        )
...
def _process_area(area: AreaConfig, config: ForecastConfig) -> None:
    ...
    if not forecast_text:
        ...
        model_label, model_ack = _model_credit(
            payload.model_ref or payload.model_id for payload in payloads
        )
        _preserve_or_render_unavailable_forecast(
            destination=destination,
        ...
        raise ForecastGenerationFailure(
            f"Area forecast unavailable for {area.name}; all configured LLM attempts failed."
        )
    if forecast_text:
        forecast_text = _trim_extra_days(
            forecast_text,
...
def _process_regional_area(area: AreaConfig, config: ForecastConfig) -> None:
    ...
    if not forecast_text:
        ...
        raise ForecastGenerationFailure(
            f"Regional forecast unavailable for {area.name}; all configured LLM attempts failed."
        )
    if forecast_text:
        forecast_text = _trim_extra_days(
            forecast_text,
...
_UNAVAILABLE_FORECAST_TEXT = (
    "**Forecast temporarily unavailable**\n\n"
    "IBF could not generate forecast text from the latest data. "
    "The next scheduled run will try again."
)


def _has_preservable_forecast_page(path: Path) -> bool:
    """Return whether an existing page contains a real forecast worth retaining."""
    if not path.exists() or _is_placeholder_output(path):
        return False
    try:
        ...
        return False
    invalid_markers = (
        "Area dataset preview for",
        "<strong>Dataset preview</strong>",
        "Forecast temporarily unavailable",
    )
    return bool(text.strip()) and not any(marker in text for marker in invalid_markers)


def _preserve_or_render_unavailable_forecast(
    *,
    destination: Path,
    display_name: str,
    issue_time: str,
    model_label: str,
    model_ack_url: Optional[str],
    map_link: Optional[str] = None,
) -> None:
    """Preserve a prior real forecast or publish a safe first-run failure page."""
    if _has_preservable_forecast_page(destination):
        logger.error(
            "Forecast generation failed for '%s'; preserving the previous forecast page at %s.",
            display_name,
            destination,
        )
        return
    ...
    logger.error(
        "Forecast generation failed for '%s' and no previous valid page exists; "
        "publishing a temporary-unavailable page at %s.",
        display_name,
        destination,
    )
    render_forecast_page(
        ForecastPage(
            ...
            forecast_text=_UNAVAILABLE_FORECAST_TEXT,
            translated_text=None,
            translation_language=None,
            ibf_context=None,
            map_link=map_link,
            model_label=model_label,
            model_ack_url=model_ack_url,
        )
    )
    ...
    for index, (role, choice, supplied_settings) in enumerate(attempts):
        ...
        try:
            ...
            if (
                ...
                and settings.provider == "lmstudio"
...
def _log_prompt_size(
    ...
    *,
    ...
    """Log prompt size before a provider call, including a rough token estimate."""
